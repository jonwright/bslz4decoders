...
f2py_start_clock();
  ...
  if (!PyArg_ParseTupleAndKeywords(capi_args,capi_keywds,\
    "OOOOO|:ippompdecoders.omp_bslz4",\
...
/*frompyobj*/
  ...
  if (capi_tmp_tmp == NULL) {
    PyObject *exc, *val, *tb;
    PyErr_Fetch(&exc, &val, &tb);
    PyErr_SetString(exc ? exc : ippompdecoders_error,"failed in converting 4th argument `tmp' of ippompdecoders.omp_bslz4 to C/Fortran array" );
    npy_PyErr_ChainExceptionsCause(exc, val, tb);
  } else {
    tmp = (unsigned_char *)(PyArray_DATA(capi_tmp_tmp));

  /* Processing variable num_threads */
    f2py_success = int_from_pyobj(&num_threads,num_threads_capi,"ippompdecoders.omp_bslz4() 5th argument (num_threads) can't be converted to int");
  if (f2py_success) {
...
#endif
  if (!PyArg_ParseTupleAndKeywords(capi_args,capi_keywds,\
    "OOOOOOO|:ippompdecoders.omp_bslz4_blocks",\
...
/*frompyobj*/
  ...
  if (capi_tmp_tmp == NULL) {
    PyObject *exc, *val, *tb;
    PyErr_Fetch(&exc, &val, &tb);
    PyErr_SetString(exc ? exc : ippompdecoders_error,"failed in converting 6th argument `tmp' of ippompdecoders.omp_bslz4_blocks to C/Fortran array" );
    npy_PyErr_ChainExceptionsCause(exc, val, tb);
  } else {
    tmp = (unsigned_char *)(PyArray_DATA(capi_tmp_tmp));

  /* Processing variable num_threads */
    f2py_success = int_from_pyobj(&num_threads,num_threads_capi,"ippompdecoders.omp_bslz4_blocks() 7th argument (num_threads) can't be converted to int");
  if (f2py_success) {
  /* Processing variable compressed_length */
  compressed_length = len(compressed);
